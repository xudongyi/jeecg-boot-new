...
public class CompanyRiskWasteController extends JeecgController<CompanyRiskWaste, ICompanyRiskWasteService> {
    ...
    public Result<?> add(@RequestBody JSONObject jsonObject) {
        CompanyRiskWaste companyRiskWaste = this.getCompanyRiskWaste(jsonObject);
        ...
        companyFileService.saveFiles(jsonObject.getString("fileList"), Constant.fileType.FILE, Constant.tables.RISKWASTE, companyRiskWaste.getId());
        return Result.ok("添加成功！");
    }

    private CompanyRiskWaste getCompanyRiskWaste(JSONObject jsonObject) {
        CompanyRiskWaste companyRiskWaste = new CompanyRiskWaste();
        companyRiskWaste.setId(jsonObject.getString("id"));
        companyRiskWaste.setCompanyId(jsonObject.getString("companyId"));
        companyRiskWaste.setCreateBy(jsonObject.getString("createBy"));
        companyRiskWaste.setCreateTime(jsonObject.getDate("createTime"));
        companyRiskWaste.setUpdateBy(jsonObject.getString("updateBy"));
        companyRiskWaste.setUpdateTime(jsonObject.getDate("updateTime"));
        companyRiskWaste.setStatus(jsonObject.getString("status"));
        companyRiskWaste.setLicenceCode(jsonObject.getString("licenceCode"));
        companyRiskWaste.setValidStarttime(jsonObject.getDate("validStarttime"));
        companyRiskWaste.setValidEndtime(jsonObject.getDate("validEndtime"));
        companyRiskWaste.setCertificateTime(jsonObject.getDate("certificateTime"));
        companyRiskWaste.setCertificateOffice(jsonObject.getString("certificateOffice"));
        companyRiskWaste.setDirtyType(jsonObject.getString("dirtyType"));
        return companyRiskWaste;
    }
        ...
        if (!StrUtil.isEmpty(companyRiskWaste.getId())) {
            ...
            if (Constant.status.NORMAL.equals(oldCompanyRiskWaste.getStatus())) {
              /*  CompanyApply companyApply = companyApplyService.findByNewId(oldCompanyRiskWaste.getId(), Constant.tables.RISKWASTE);
                ...
                companyApplyService.updateById(companyApply);
                //删除原有的
                companyFileService.remove(new QueryWrapper<CompanyFile>().lambda().eq(CompanyFile::getFromTable, Constant.tables.RISKWASTE)
                        .eq(CompanyFile::getTableId, companyRiskWaste.getId()));
            }
        } else {
            //新增
            companyRiskWasteService.save(companyRiskWaste);
    ...
    public Result<?> queryFiles(@RequestParam(name = "id", required = true) String id) {
        List<Map<String, String>> result = companyFileService.getFileMaps(id, Constant.tables.RISKWASTE);
